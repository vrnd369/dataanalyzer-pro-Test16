...
export function ChartView({ 
  ...
  const chartData = React.useMemo(() => {
    try {
      ...
      if (selectedFields.length > 0) {
        numericFields = numericFields.filter(f => selectedFields.includes(f.name));
      }

      if (numericFields.length === 0) {
        setError('No numeric fields available for visualization');
        return null;
      }
      ...
      if (type === 'scatter') {
        if (numericFields.length < 2) {
          setError('Scatter plot requires at least two numeric fields');
          return null;
        }
        ...
        return scatterData as ChartData<'scatter'>;
      }
      ...
      // Additional statistical data if enabled
      if (showStats) {
        baseDatasets.push(
          {
            label: 'Median Values',
  ...
  }, [data, type, selectedFields, showStats, colors, setError]);
