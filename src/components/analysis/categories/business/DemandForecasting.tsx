...
export function DemandForecasting({ data }: DemandForecastingProps) {
  ...
  const normalizedData = useMemo(() => {
    ...
    const length = dateField.value.length;
  ...
  const calculateMetrics = () => {
    if (actualData.length < 2 || forecastData.length <= actualData.length) return null;
    
    const comparableData = actualData.slice(-Math.min(actualData.length, 5));
    const forecastPoints = forecastData.slice(0, comparableData.length);
    
    let sumSquaredErrors = 0;
    let sumAbsoluteErrors = 0;
    let sumAbsolutePercentageErrors = 0;
    
    comparableData.forEach((point, i) => {
      const error = forecastPoints[i].value - point.value;
      sumSquaredErrors += error * error;
      sumAbsoluteErrors += Math.abs(error);
      sumAbsolutePercentageErrors += Math.abs(error) / point.value;
    });
    
    const mse = sumSquaredErrors / comparableData.length;
    const rmse = Math.sqrt(mse);
    const mae = sumAbsoluteErrors / comparableData.length;
    const mape = (sumAbsolutePercentageErrors / comparableData.length) * 100;
        ...
        <div className="space-y-6">
          ...
          {metrics && (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <Card className="p-4">
                <h4 className="text-sm font-medium text-black">Root Mean Squared Error</h4>
                <p className="text-2xl font-bold mt-2 text-black">{metrics.rmse}</p>
              </Card>
              <Card className="p-4">
                <h4 className="text-sm font-medium text-black">Mean Absolute Error</h4>
                <p className="text-2xl font-bold mt-2 text-black">{metrics.mae}</p>
              </Card>
              <Card className="p-4">
                <h4 className="text-sm font-medium text-black">Mean Absolute % Error</h4>
                <p className="text-2xl font-bold mt-2 text-black">{metrics.mape}%</p>
              </Card>
              <Card className="p-4">
                <h4 className="text-sm font-medium text-black">Forecast Accuracy</h4>
                <p className="text-2xl font-bold mt-2 text-black">{metrics.accuracy}%</p>
              </Card>
            </div>
          )}
